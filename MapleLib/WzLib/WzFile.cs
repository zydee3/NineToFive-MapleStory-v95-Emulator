...
namespace MapleLib.WzLib {
    ...
    public class WzFile : WzObject {
        ...
        public short Version {
            get { return mapleStoryPatchVersion; }
        ...
        public WzObject GetObjectFromPath(string relativePath, bool checkFirstDirectoryName = true) {
            string[] sp = relativePath.Split("/".ToCharArray());

            if (checkFirstDirectoryName) {
                if (!sp[0].Equals(wzDir.name, StringComparison.OrdinalIgnoreCase)
                    && sp[0].Equals(wzDir.name.Remove(name.Length - 3), StringComparison.OrdinalIgnoreCase)) {
                    return null;
                }
            }

            if (sp.Length == 1) return WzDirectory;
            WzObject curObj = WzDirectory;
            for (int i = (checkFirstDirectoryName ? 1 : 0); i < sp.Length; i++) {
                ...
                switch (curObj) {
                    ...
                    case WzImageProperty property:
                        switch (property.PropertyType) {
                            case WzPropertyType.Canvas:
                                curObj = ((WzCanvasProperty) curObj)[sp[i]];
                                continue;
                            case WzPropertyType.Convex:
                                curObj = ((WzConvexProperty) curObj)[sp[i]];
                                continue;
                            case WzPropertyType.SubProperty:
                                curObj = ((WzSubProperty) curObj)[sp[i]];
                                continue;
                            case WzPropertyType.Vector:
                                if (sp[i] == "X")
